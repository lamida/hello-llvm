...
cd $title/dot-scarr
/home/lamida/github/llvm/llvm-project/llvm/cmake-build-release/bin/opt ../../$title.ll -passes=mycfg,dot-cfg
ls *.dot | xargs -I{} dot -Tpng {} -o {}.png
# generate png for hidden files too
ls .*.dot | xargs -I{} dot -Tpng {} -o {}.png
cd ../..

mkdir -p $title/dot-scarr-inline
cd $title/dot-scarr-inline
